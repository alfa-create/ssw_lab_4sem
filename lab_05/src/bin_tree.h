...
void bin_tree<T>::Push(T data) {
    ...
    size++;
    if ( root->value > data ) root->ptrLeft = BalanceBranch( root->ptrLeft );
    else root->ptrRight = BalanceBranch( root->ptrRight);
...
void bin_tree<T>::Pop() {
    ...
    if ( size == 1 ) {
        ...
        while ( current->ptrLeft != nullptr || current->ptrRight != nullptr ) {
            previous = current;
            if (current->ptrLeft == nullptr) current = current->ptrRight;
            else current = current->ptrLeft;
        }
        if ( previous->value > current->value ) previous->ptrLeft = nullptr;
        else previous->ptrRight = nullptr;
        bal = current->value;
        delete current;
    }
    if ( root->value > bal ) root->ptrLeft = BalanceBranch( root->ptrLeft );
    else root->ptrRight = BalanceBranch( root->ptrRight );
    root = BalanceNode(root);
    size--;
}
